...
type ProcessGroupCommand struct {
	Cmd  *exec.Cmd
	Stop chan bool // Stop is a channel that receives stop requests.
}
...
func NewProcessGroupCommand(stop chan bool, logFile *os.File, name string, args []string) *ProcessGroupCommand {
...
func (group *ProcessGroupCommand) Wait() (bool, error) {
	...
	end := make(chan bool) // this will be closed when process exits normally
